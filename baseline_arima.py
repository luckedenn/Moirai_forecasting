...
def rolling_forecast_arima(series: pd.Series, pdt: int, frac_test: float = 0.15, fast_mode: bool = True):
    n = len(series)
    
    # Jika fast mode, batasi maksimal 8 windows
    max_windows = 8 if fast_mode else None
    TEST = choose_test_len(n, pdt, frac_test, max_windows) if fast_mode else choose_test_len(n, pdt, frac_test)
    windows = TEST // pdt
    ...
    for w in range(windows):
        ...
        model = auto_arima(
            ...
            max_p=3, max_q=3,          # kurangi dari 5 ke 3
            start_P=0, start_Q=0,
            max_P=1, max_Q=1,          # kurangi dari 2 ke 1
            ...
            maxiter=20,                # kurangi dari 50 ke 20
...
def run_arima(name: str, csv: str, freq: str, pdt: int, outdir_root: str = "results_baseline_arima", fast_mode: bool = True):
    ...
    rows, metrics = rolling_forecast_arima(series, pdt=pdt, frac_test=0.15, fast_mode=fast_mode)
...
if __name__ == "__main__":
    ...
    DATASETS = [
        ("weather_melbourne", "data/weather_melbourne/weather_melbourne_full.csv", "D", 24),
        ("finance_aapl", "data/finance_aapl/finance_aapl_full.csv", "D", 24),   # daily (lebih stabil dari business day)
        ("co2_maunaloa_monthly", "data/co2_maunaloa_monthly/co2_maunaloa_monthly_full.csv", "M", 12),
    ]
