...
def load_standardized_results():
    ...
    try:
        ...
        print(f" Zero-shot loading failed: {e}")
    ...
    try:
        ...
        print(f" Few-shot MoE loading failed: {e}")
...
def create_standardized_summary_table(df):
    ...
    for dataset in datasets:
        ...
        print(f"{'Rank':<6}{'Model':<15}{'MAE':<12}{'sMAPE (%)':<12}{'Windows':<8}")
        print("-" * 60)
        
        for i, (_, row) in enumerate(dataset_df.iterrows(), 1):
            rank_emoji = "1" if i == 1 else "2" if i == 2 else "3" if i == 3 else f"{i}."
            
            mae_str = f"{row['MAE']:.3f}±{row['MAE_std']:.3f}"
            smape_str = f"{row['sMAPE']:.2f}±{row['sMAPE_std']:.2f}"
            
            print(f"{rank_emoji:<6}{row['Model']:<15}{mae_str:<12}{smape_str:<12}{row['Windows']:<8}")
...
def create_standardized_visualizations(df):
    ...
    metrics = ['MAE', 'sMAPE']
    titles = ['Mean Absolute Error', 'Symmetric MAPE (%)']
...
def overall_analysis(df):
    ...
    print(f"\n STATISTICAL SUMMARY:")
    print("-" * 30)
    print(df.groupby('Model')[['MAE', 'sMAPE']].agg(['mean', 'std']).round(3))
